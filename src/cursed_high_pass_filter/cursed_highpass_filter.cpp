...
float cursed_highpass(float sample) {
  ...
  // aggregation If any value is exactly 0, skip exponentiation and set to 0
  // Otherwise, perform exponentiation even if small
  float last_output_term =
      (abs_last_output == 0.0f) ? 0.0f : std::pow(abs_last_output, alpha);
  float sample_term = (abs_sample == 0.0f) ? 0.0f : std::pow(abs_sample, alpha);
  float last_input_term =
      (abs_last_input == 0.0f) ? 0.0f : std::pow(abs_last_input, 1.0f - alpha);
